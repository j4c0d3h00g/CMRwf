...
crm_functional <- function (formula, data, maxiter = 100, tolerance = 0.01, outlyingness.factor = 1,
                            ...
                            weightfunction = "hampel", weightthreshold = 1, parameters) {
  # -----------------------------------------------------------------------------------------------
  ...
  ## Calculate residuals and derive case weights with weight function
  r <- robreg$residuals
  r <- scaleResidualsByMAD(r)

  if (weightfunction == "hampel") {
    q1 <- parameters[1]
    q2 <- parameters[2]
    q3 <- parameters[3]
    wr <- HampelWeightFunction(r, q1, q2, q3)
  } else if (weightfunction == "tukey") {
    q <- parameters[1]
    wr <- TukeyWeightFunction(r, q)
  } else if (weightfunction == "huber") {
    q <- parameters[1]
